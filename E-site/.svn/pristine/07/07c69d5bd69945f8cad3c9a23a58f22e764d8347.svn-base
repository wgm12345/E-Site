...
<c:if test="${customerList==null}">
	<c:redirect
		url="/servlet/CustomerManageServlet?fromPage=/Admin/Customer_List.jsp&act=customerList"></c:redirect>
</c:if>
...
<script language="javascript">
	...
	function confirm_redirect(suser) {
		if (confirm("您确定要删除该会员账号吗?")) {
				location.href = "<c:url value='/servlet/CustomerManageServlet?fromPage=/Admin/Customer_List.jsp&del="
						+ suser + "&act=delcustomer' />"
		}
	}

	function showinfo(suser) {
		location.href = "<c:url value='/Admin/Customer_Info.jsp?suser=' />"
				+ suser;
	}
	function editinfo(suser) {
		location.href = "<c:url value='/Admin/Customer_Einfo.jsp?suser=' />"
				+ suser;
	}
</script>
</head>
<body>
	<div class="form-div">
		<form action="<c:url value="/servlet/CustomerManageServlet"/>"
			name="searchForm">
			<img src="<c:url value="/Admin/images/icon_search.gif" />" width="26"
				height="22" border="0" alt="SEARCH" />&nbsp;会员用户名 &nbsp; <input
				type="text" name="suser" value="${param.suser}" /> &nbsp;会员员真实姓名
			&nbsp; <input type="text" name="sname" value="${param.sname}" />&nbsp;
			<input type="submit" value=" 搜索 " class="button" /> <input
				type="hidden" name="act" value="customerList" /> <input type="hidden"
				name="fromPage" value="/Admin/Customer_List.jsp" /> <input
	...
	<div class="list-div" id="listDiv">
		...
		<table cellpadding="0" cellspacing="1">
			...
			<c:forEach var="userBean" items="${customerList}">
				<tr>
					<td align="center">${userBean.cid}</td>
					<td align="center">${userBean.cname}</td>
					<td align="center">${userBean.cemail}</td>
					<td align="center">${userBean.cphone}</td>
					<td align="center"><span
						onclick="showinfo('${userBean.cid}');" style="cursor:hand;"
						title="查看会员资料"> <img
							src="<c:url value="/Admin/images/icon_view.gif"/>" border="0"
							height="16" width="16" /></span> <span
						onclick="editinfo('${userBean.cid}');" style="cursor:hand;"
						title="编辑会员资料"> <img
							src="<c:url value="/Admin/images/icon_edit.gif"/>" border="0"
							height="16" width="16" /></span> <span
						onclick="confirm_redirect('${userBean.cid}');" style="cursor:hand;"
						style="cursor:hand;" title="删除会员"> <img
